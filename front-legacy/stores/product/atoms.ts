...
export const setScrapedProductsAtom = atom(
  null,
  async (get, set, products: Product[], query: string) => {
...
export const getMainProductsAtom = atom(
  null,
...
export const productReviewAtom = atom<ProductReview>({
  reviews: [],
} as ProductReview);

export const setProductReviewAtom = atom(
  null,
  async (get, set, reviews: string[]) => {
    set(productReviewAtom, { reviews });
  }
);

export const productDetailAtom = atom<GetProductDetailResponse | void>(
  undefined
);
...
export const setProductAtom = atom(null, async (get, set, product: Product) => {
  ...
  if (productDetail?.product?.url === product.url) {
    ...
    if (
      !deepEqual(productDetail.product, product) &&
      product.name &&
      product.price
    ) {
...
export const getProductDetailAtom = atom(
  null,
  async (get, set, product: Product) => {
    ...
    if (
      ...
      get(productDetailAtom)?.product?.url === product.url
    ) {
...
// AI 포미 답변 생성
export const getProductAIAnswerAtom = atom(
  null,
  async (get, set, question: string) => {
    ...
    const product = get(productDetailAtom)?.product!;
    const reviews = get(productReviewAtom).reviews;
    const response = await GetProductAIAnswerAPI({
      product,
      reviews,
      question,
    });
