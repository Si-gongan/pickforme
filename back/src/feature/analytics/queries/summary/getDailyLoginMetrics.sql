-- @target_date를 기준으로 로그인 관련 모든 메트릭을 집계합니다.
...
USING (
  ...
  WHERE
    ...
    AND event_name IN ('login_attempt', 'login_success', 'register_success')
) AS source
...
WHEN NOT MATCHED THEN
  INSERT (summary_date, login_attempt_count, login_success_count, kakao_login_attempt_count, google_login_attempt_count, apple_login_attempt_count, register_success_count)
  VALUES (source.summary_date, source.login_attempt_count, source.login_success_count, source.kakao_login_attempt_count, source.google_login_attempt_count, source.apple_login_attempt_count, source.register_success_count);