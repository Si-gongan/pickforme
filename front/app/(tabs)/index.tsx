...
export default function DiscoverScreen() {
  ...
  // 2024-08-26(월) TODO
  const setIsShowVersionUpdateAlarmModal = useSetAtom(
    isShowVersionUpdateAlarmModalAtom
  );
  ...
  useEffect(() => {
    ...
    if (
      applicationVersion &&
      isVersionLessThan(applicationVersion, APPLICATION_VERSION)
    ) {
  ...
  return (
    <View style={styles.container}>
      ...
      {isSearching ? (
        <Text style={styles.loading} ref={searchLoadingRef}>
          검색하신 상품을 로딩중이에요.
        </Text>
      ) : !!query.length ? (
        // 검색 결과 섹션
          ...
          <View style={styles.searchStatus}>
            ...
            >
              <Text style={styles.productCount}>
                총 {searchResult?.products?.length}건
              </Text>
            </View>
            ...
            {!!searchResult?.products.length && (
              <FlatList
                ...
                ItemSeparatorComponent={() => (
                  <View style={styles.seperator} accessible={false} />
                )}
                // onEndReached={() => handleSearchMore(query)}
              />
            )}
            {!searchResult?.products?.length && (
              <Text style={styles.loading}>검색결과가 없습니다.</Text>
            )}
          </ScrollView>
        </>
      ) : (
          ...
          {mainProducts.local
            ...
            .map((section) => (
              <View
                ...
                key={`discover-main-section-${section.name}-${section.order}`}
              >
                <Text
                  style={[styles.sectionTitle, styles.horizontalPadder]}
                  accessible
          ...
          {!!mainProducts.random.length && (
            <View style={styles.section}>
              ...
              <FlatList
                ...
                renderItem={({ item: product }) => (
                  <ProductCard
                    ref={focus.random === product.url ? focusRef1 : undefined}
                    product={product}
                    type="bestcategories"
                  />
                )}
                ...
                ListFooterComponent={
                  mainProducts.random.length > length.random
                    ? () => (
                        <MoreButton onClick={() => handleClickMore("random")} />
                      )
                    : undefined
                }
              />
          ...
          {!!mainProducts.special.length && (
            <View style={styles.section}>
              ...
              <FlatList
                ...
                renderItem={({ item: product }) => (
                  <ProductCard
                    ref={focus.special === product.url ? focusRef2 : undefined}
                    product={product}
                    type="goldbox"
                  />
                )}
                ListFooterComponentStyle={styles.listFooter}
                ListFooterComponent={
                  mainProducts.special.length > length.special
                    ? () => (
                        <MoreButton
                          onClick={() => handleClickMore("special")}
                        />
                      )
                    : undefined
                }
