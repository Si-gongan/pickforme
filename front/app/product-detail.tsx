...
const ProductDetailScreen: React.FC<ProductDetailScreenProps> = () => {
  ...
  const request = requests.filter((req) => req.product).find((req) => decodeURIComponent(req.product!.url) === productUrl);
  ...
  const product = request?.product
    ...
    || [...mainProducts.local.map((section) => section.products).flat(), ...mainProducts.special, ...mainProducts.random].find(
      ({ url }) => decodeURIComponent(url) === productUrl
    )
    || already
    || { url: productUrl } as Product;
  const isLocal = mainProducts.local.map((section) => section.products).flat().find(({ url }) => decodeURIComponent(url) === productUrl) !== undefined;
              ...
              <View style={styles.priceWrap} accessible accessibilityRole="text">
                {productDetail?.product ? (
                  <>
                    ...
                    <Text style={styles.price} accessibilityLabel={`현재 가격 ${numComma(productDetail?.product?.price ?? 0)}원`}>
                    ...
                    {(productDetail?.product?.origin_price ?? 0) !== 0 &&
                      productDetail?.product?.price !== productDetail?.product?.origin_price && (
                        <Text style={styles.origin_price} accessibilityLabel={`할인 전 가격 ${numComma(productDetail?.product?.origin_price ?? 0)}원`}>
              ...
              <View style={styles.table}>
                <View style={styles.tableList}>
                  ...
                  <View style={styles.tableRow} accessible>
                    ...
                    <Text style={styles.tableItem}>{Math.floor((productDetail?.product?.ratings ?? 0) / 20 * 10) / 10} 점</Text>
                  </View>
                </View>
              </View>
            </View>
...
const ActionButtons: React.FC<ActionButtonsProps> = ({
  ...
  <View style={styles.buttonWrap}>
    ...
    {product?.platform === 'thezam' ? (
      <View style={styles.buttonOuter}>
        <Button title="대리구매 요청하기" onPress={handleClickContact} style={[styles.button, styles.button2]} color="tertiary" size="small" disabled={!product} />
      </View>
    ) : (
      <View style={styles.buttonOuter}>
        <Button
    ...
    {isWish ? (
      <Pressable onPress={handleClickWish} accessible accessibilityLabel="위시리스트 제거" accessibilityRole="button" disabled={!product}>
        <Image style={styles.heartIcon} source={require('../assets/images/discover/icHeartFill.png')} />
      </Pressable>
    ) : (
      <Pressable onPress={handleClickWish} accessible accessibilityLabel="위시리스트 추가" accessibilityRole="button" disabled={!product}>
