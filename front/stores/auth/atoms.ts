...
export const isOnboardingFinishedAtom = atomWithStorage<"true" | "false">(
...
export const handleLoginResultAtom = atom(
  null,
  async (get, set, data: LoginResponse) => {
...
export const loginGoogleAtom = atom(
  null,
  async (get, set, params: GoogleLoginParams) => {
    const { data } = await GoogleLoginAPI(params);
    set(handleLoginResultAtom, data);
  }
);

export const loginAppleAtom = atom(
  null,
  async (get, set, params: AppleLoginParams) => {
    const { data } = await AppleLoginAPI(params);
    set(handleLoginResultAtom, data);
  }
);

export const loginKakaoAtom = atom(
  null,
  async (get, set, params: KakaoLoginParams) => {
    try {
...
export const setPushTokenAtom = atom(
  null,
  async (get, set, params: SetPushTokenParams) => {
    await SetPushTokenAPI(params);
  }
);

export const setPushSettingAtom = atom(
  null,
  async (get, set, params: SetPushSettingParams) => {
    const userData = await get(userDataAtom);
    if (userData) {
